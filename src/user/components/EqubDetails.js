...
import { Card, CardContent, Typography, Button, Alert, Box, IconButton } from '@mui/material';
    ...
    const fetchEqubDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/equbs/${id}`, // Fetch equb details by ID
          {
            headers: {
  ...
  return (
    <Box sx={{ padding: 3 }}>
      ...
      <Card>
        <CardContent>
          {/* Back button */}
          <IconButton onClick={() => navigate('/myequb')} color="primary" aria-label="back">
            <ArrowBack />
            <Typography variant="body2">Back</Typography>
          </IconButton>
          
          <Typography variant="h4">{equb.name}</Typography> {/* Display equb name */}
          <Typography variant="body1">{equb.description}</Typography> {/* Display equb description */}
          <Typography variant="body2">Frequency: {equb.frequency}</Typography>
          <Typography variant="body2">Contribution: {equb.contributionAmount}</Typography>
