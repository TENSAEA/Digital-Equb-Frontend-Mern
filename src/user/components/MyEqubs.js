...
const MyEqubs = () => {
    ...
    const fetchUserEqubs = async () => {
      try {
        ...
        const response = await axios.get(
          'http://localhost:5000/api/equbs/my-equbs', // Endpoint for user's equbs
          {
            headers: {
  ...
  return (
    <Box sx={{ flexGrow: 1, padding: 3, ml: 30 }}>
      {message && <Alert severity={isError ? 'error' : 'success'}>{message}</Alert>} {/* Display error or success message */}
      ...
      <Grid container spacing={3}>
        {equbs.map((equb) => (
          <Grid item xs={12} sm={6} md={4} key={equb._id}>
            <Card>
              <CardContent>
                <Typography variant="h6">{equb.name}</Typography> {/* Display equb name */}
                <Typography variant="body2">Admin: {equb.admin.fname} {equb.admin.lname}</Typography> {/* Display admin name */}
                <Typography variant="body2">Members: {equb.members.length}</Typography> {/* Display number of members */}
              </CardContent>
            </Card>
          </Grid>
        ))}
