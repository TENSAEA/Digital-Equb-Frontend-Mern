...
export const AuthProvider = ({ children }) => {
  ...
  const storedToken = localStorage.getItem('token'); // Ensure correct key
  const storedUser = localStorage.getItem('user'); // Ensure correct key

  const [auth, setAuth] = useState({
    token: storedToken || null,
    user: storedUser ? JSON.parse(storedUser) : null, // Parse the user data
  });
